...
INPUT_CSV   = os.path.join(os.path.dirname(ROOT), 'analysis_ready.csv')
...
for pol_lab in ['NO2', 'PM2.5', 'PM10']:
    ...
    im = ax2.imshow(Z, aspect='auto', origin='lower',
                    extent=[pcs.min(), pcs.max(), lags_v.min()-0.5, lags_v.max()+0.5],
                    cmap='viridis')
    ax2.set_xticks(pcs); ax2.set_yticks(lags_v)
    ax2.set_xlabel('pc threshold'); ax2.set_ylabel('lag (weeks)')
    ax2.set_title(f'Heatmap — {pol_lab} -> respiratory')
    fig.colorbar(im, ax=ax2)
    # Annotate cells
    for i, L in enumerate(lags_v):
        for j, pc in enumerate(pcs):
            ax2.text(pc, L, f"{Z[i,j]:.2f}", ha='center', va='center',
                     color='white' if Z[i,j] < (np.nanmax(Z)+np.nanmin(Z))/2 else 'black',
                     fontsize=9)
    fig.tight_layout()
